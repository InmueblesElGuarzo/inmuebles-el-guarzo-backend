...
import { RejectPublicationRequestInteractor } from './reject-publication-request.interactor';
...
const REJECT_INPUT = {
  ...
  decidedByAdminId: ADMIN_UUID,
  decisionMotive: 'No cumple los requisitos mínimos establecidos.',
};

describe('RejectPublicationRequestInteractor.execute — reject exitoso', () => {
  it('should return Result.ok with REJECTED status', async () => {
    ...
    const interactor = new RejectPublicationRequestInteractor(repo, eventBus);
    const result = await interactor.execute(REJECT_INPUT);

    expect(result.isSuccess).toBe(true);
    expect(result.value.status).toBe(PublicationRequestStatusValue.REJECTED);
  });
});

describe('RejectPublicationRequestInteractor.execute — solicitud no encontrada', () => {
...
describe('RejectPublicationRequestInteractor.execute — motivo vacío', () => {
  it('should return Result.fail with DecisionMotiveRequiredException', async () => {
    const repo = buildMockRepo();
    const eventBus = buildMockEventBus();
    repo.findById.mockResolvedValue(
      Maybe.some(buildRequest(PublicationRequestStatusValue.UNDER_REVIEW)),
    ...
    const interactor = new RejectPublicationRequestInteractor(repo, eventBus);
    const result = await interactor.execute({ ...REJECT_INPUT, decisionMotive: '   ' });

    expect(result.isFailure).toBe(true);
    expect(result.error).toBeInstanceOf(DecisionMotiveRequiredException);
  });
});

describe('RejectPublicationRequestInteractor.execute — solicitud ya decidida', () => {
